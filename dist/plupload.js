...
/***/ function(module, exports, __webpack_require__) {
	...
	var plupload = __webpack_require__(513);

	;(function() {
		var myPlupload = new plupload.Uploader({
			browse_button: $('.btn-browse')[0],
			url: '',
			filters: {
				mime_types: [{
					title: 'Image files',
		...
		myPlupload.bind('FilesAdded', function(_uploader, files) {
			if (files.length > 0) {
				_uploader.setOption('multipart_params', {
					uploadType: 'Adpic',//传给后端的参数
				});
				_uploader.start();
			} else {
				alert('请选择文件');
		...
		myPlupload.bind('FileUploaded', function(_uploader, file, response) {
			var resp = $.parseJSON(response);//将后端返回数据解析
			if (resp.successSign) {
				alert('图片上传成功');
			} else {
				alert(resp.message);
